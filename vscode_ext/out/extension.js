  ...
  static async load(input) {
    ...
    if (input instanceof Uint8Array) {
      ...
      if (globalThis.process?.versions.node) {
        const fs2 = await Promise.resolve(/* import() */).then(__webpack_require__.t.bind(__webpack_require__, 943, 23));
...
const web_tree_sitter_1 = __webpack_require__(78);
...
async function parseAllPythonFiles() {
...
async function saveParsingResults(results) {
...
function extractSemanticRelationships(results) {
    const relationships = {
        generatedAt: new Date().toISOString(),
        functions: [],
        classes: [],
...
async function activate(context) {
    ...
    try {
        ...
        const Python = await web_tree_sitter_1.Language.load(pythonWasmData);
        pythonParser = new web_tree_sitter_1.Parser();
    ...
    const indexWorkspaceCommand = vscode.commands.registerCommand('badger.indexWorkspace', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Indexed ${totalFiles} Python files: ${totalFunctions} functions, ${totalClasses} classes. Results saved to .badger-index/`);
    ...
    const parseCurrentFileCommand = vscode.commands.registerCommand('badger.parseCurrentFile', async () => {
        ...
        try {
            ...
            outputChannel.show();
            vscode.window.showInformationMessage(`Parsed ${result.functions.length} functions, ${result.classes.length} classes`);
        }
        catch (error) {
    ...
    context.subscriptions.push(indexWorkspaceCommand, parseCurrentFileCommand, queryContextCommand, updateGraphCommand, fileSaveListener);
